...
def to_h5(mimic, config) -> None:
    ...
    for _, row in cohort.iterrows():
        ...
        for master_path in master_paths:
            record_meta = get_record_meta(master_path, record_dir, row['fs'],
                            target_fs, chunk_duration)
            ...
            index_meta.append({
                ...
                'has_labs':    has_labs,
                **icu_meta,
                **{f'{sig}': int(sig in signal_map) for sig in TARGET_SIGNALS},
            })
            index_meds_labs.append({
                'stay_id': int(stay_id),
                **{f'{cat}': int(cat in med_cat_feats)  for cat in MED_CATEGORIES},
                **{f'{med}': int(med in med_feats)  for med in INPUT_FEATURES},
                **{f'{lab}': int(lab in lab_feats)  for lab in LAB_MAP.values()},
            })
            index_codes.append({
                'stay_id': int(stay_id),             
    ...
    index_meta = pd.DataFrame(index_meta)
    index_meta.to_csv(f"{config['paths'][f'mimic{mimic}']['output_dir']}/index_meta.csv", index=False)
    index_meds_labs = pd.DataFrame(index_meds_labs)
    index_meds_labs.to_csv(f"{config['paths'][f'mimic{mimic}']['output_dir']}/index_meds_labs.csv", index=False)
    index_codes = pd.DataFrame(index_codes)
    index_codes.to_csv(f"{config['paths'][f'mimic{mimic}']['output_dir']}/index_codes.csv", index=False)
    return index_meta
...
class ICUDataset(IterableDataset):
    def __init__(self, 
        ...
        self.float_features = lab_labels + [f'{lab_label}_stale' for lab_label in lab_labels] + [f'{med_label}_ratenorm' for med_label in med_labels] + ['norepi_eq']
        self.int_features = code_labels + [f'{lab_label}_abnorm' for lab_label in lab_labels] + [f'{med_label}_bolus' for med_label in med_labels if med_label in BOLUS_FEATURES] + [f'{med_category}_on' for med_category in med_categories]
    ...
    def _forward_fill(self, ehr_df, timestamps):
        #Forward fill lab values and abnormality flag, calculate staleness in minutes
        ...
        for lab in present_labs:
            ...
            stale_cols[f'{lab}_stale'] = (chunk_times - last_draw_time).dt.total_seconds() / 60
